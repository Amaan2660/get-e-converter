...
# Location of bundled template
TEMPLATE_PATH = Path(__file__).with_name("template.csv")
...
uploaded_template = st.sidebar.file_uploader(
    "Upload full 98‑column template CSV (optional)", type=["csv"]
)

# Use bundled template by default
if uploaded_template:
    template_cols = load_template(uploaded_template)
    st.session_state["template_cols"] = template_cols
else:
    if TEMPLATE_PATH.exists():
        template_cols = load_template(TEMPLATE_PATH)
    else:
        template_cols = BASIC_COLS
    st.session_state["template_cols"] = template_cols
...
if raw_file:
    ...
    if st.button("Convert to Import CSV"):
        ...
        for _, r in raw_df.iterrows():
            ...
            pickup_time = (
                (dt - timedelta(minutes=10)).strftime("%d/%m/%Y %H:%M") if dt else ""
            )
        ...
        st.success(
            f"Generated {len(df_out)} rows \u2713 (using {len(template_cols)}‑column template)"
        )
